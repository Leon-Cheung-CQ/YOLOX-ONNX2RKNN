...
def main():
    ...
    print('--> Loading model')
    ret = rknn.load_onnx(model=config['parameters'][model_type]['model'])
    if ret != 0:
    ...
    if model_type != 'rknn':
        ...
        print('acc_eval')
        rknn.accuracy_analysis(inputs='./acc_eval.txt')
        print('acc_eval done!')
    ...
    # initialization rknn environment
    ...
    # Set input img
    origin_img = cv2.imread(config['img']['path'])
    img, ratio = preprocess(origin_img, test_size)
    ...
    predictions = demo_postprocess(outputs[0], test_size)[0]
    ...
    cv2.imshow("post process result", origin_img)
    cv2.waitKeyEx(0)
    cv2.imwrite("postresult.jpg", origin_img)
